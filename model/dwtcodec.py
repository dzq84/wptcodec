...
from utils.tools import audio_to_mel,batch_wdt
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.block = [WNConv1d(8, d_model, kernel_size=7, padding=3)]
...
class CODEC(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        audio_data = batch[:,0:1,:]
        nodes = batch_wdt(audio_data,wavelet='db4',mode='periodization')
        #nodes = batch_wdt(audio_data)
        nodes_data = self.preprocess(nodes, self.sample_rate)

        z = self.encoder(nodes_data)
